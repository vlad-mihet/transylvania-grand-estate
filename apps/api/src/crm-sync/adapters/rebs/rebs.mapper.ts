...
export type MapResult =
  ...
  // `externalId` is carried on skips when it's known, so the orchestrator can
  // record the listing as SEEN-but-skipped and exclude it from the
  // unpublish-candidate set (a skip must never be mistaken for "deleted").
...
export function mapRebsProperty(raw: RebsProperty): MapResult {
  ...
  // From here on the id is known — carry it on every skip.
  const skip = (reason: string): MapResult => ({
    ok: false,
    reason: `${externalId}: ${reason}`,
    externalId,
  });

  if (!truthy(raw.for_sale)) return skip('not for sale');

  // Reserved / withdrawn: REBS sets `availability` falsy. Absent → assume
  // available (many records omit it). Importing a reserved unit as `available`
  // would mislead buyers, so skip it (and keep it out of the delete set).
  if (raw.availability != null && !truthy(raw.availability)) {
    return skip('not available (reserved/withdrawn)');
  }

  const price = num(raw.price_sale);
  if (price == null || price <= 0) return skip('no sale price');

  // Currency is REQUIRED — no silent EUR default. A RON-priced listing missing
  // `currency_sale` would otherwise be stored at face value as EUR (~5× too
  // high). Mirror `BrandRouteService.toEur`, which only trusts EUR/RON.
  const currency = str(raw.currency_sale).toUpperCase();
  if (currency !== 'EUR' && currency !== 'RON') {
    return skip(`missing/unsupported currency "${str(raw.currency_sale)}"`);
  }

  const type = mapPropertyType(raw.property_type);
  if (!type) {
    return skip(`unmapped property_type "${raw.property_type ?? ''}"`);
  }

  const cityName = str(raw.city);
  if (!cityName) return skip('no city');
