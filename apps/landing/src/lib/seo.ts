...
/**
 * Canonical origin resolution chain. Same shape as revery's seo.ts so
 * preview deployments / Vercel aliases are treated as non-canonical and
 * automatically `noindex` until the operator wires a custom domain via
...
function resolveOrigin(): { url: string; canonical: boolean } {
  ...
  return { url: "http://localhost:3050", canonical: false };
}
...
/**
 * Landing has no per-locale path rewrites (every URL is identity-mapped),
 * so `localizedPath` is a simple `/<locale><path>` builder. `localePrefix`
 * is "always", so the prefix is unconditional.
 */
...
export function createMetadata(opts: CreateMetadataOptions): Metadata {
  ...
  // Per-locale convention-based OG image default; pages can override with
  // an explicit `image` URL (property hero, article cover, etc.).
