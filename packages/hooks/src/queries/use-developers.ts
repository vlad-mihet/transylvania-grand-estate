...
export function useDevelopers(
  locale: string,
  params: { featured?: boolean; city?: string } = {},
) {
  ...
  const qs = q.toString();
  return useQuery({
    queryKey: ["developers", locale, params],
    queryFn: () => fetchApi<ApiDeveloper[]>(`/developers${qs ? `?${qs}` : ""}`),
    staleTime: 5 * 60_000,
  });
}

export function useDeveloper(locale: string, slug: string | null) {
  return useQuery({
    queryKey: ["developer", locale, slug],
