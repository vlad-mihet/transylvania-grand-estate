# QA Report — `feature/revery`

**Branch:** `feature/revery`
**Date:** 2026-04-18
...
2. **[Major] Lint fails** — 3 errors + 3 warnings, all in `apps/revery/src/components/property/`. Two confirmed `react-hooks/set-state-in-effect` errors. CI does not currently gate on lint, so this is not a release blocker *today* — but it is a real regression that will bite when gating turns on.
...
Plus Minors: missing fr/de translations on the new bank-rates admin screens; stale DB diacritics in city descriptions; one intentional `Common.propertyTypes.chalet` asymmetry in Revery locales.

**If (1) lands before merge: REVERY READY TO MERGE.** (2) and (3) are acceptable follow-ups if team accepts the tradeoffs.
...
| 4 | All 4 apps start without errors | **PASS (HTTP)** — landing:3000, admin:3001, revery:3002 all return correct redirects; api:3333/api/v1/health → 200. Console-level errors remain **UI-UNVERIFIED**. |
| 5 | middleware→proxy rename orphan check | **PASS** — Next.js 16.1.6 natively supports `proxy.ts`. Admin auth guard works. Landing/Revery locale routing works. Confirmed at the compiled-middleware level (`.next/dev/server/middleware.js` wraps `apps/admin/src/proxy.ts`). |
| 6 | Brand tier isolation enforced | **PASS** — `X-Site: TGE_LUXURY` → 24 luxury only; `REVERY` → 15 affordable only; `ADMIN` → 39 both. `FAKESITE` → 400. Client-side tampering `?tier=luxury` on REVERY still returns only affordable. `X-Site-Resolved` header matches per request. |
| 7 | Landing shows only luxury; Revery only affordable | **API PASS / UI-UNVERIFIED** — Origin-based resolution works for all three dev origins. Actual DOM rendering requires browser. |
...
| #2 Blocker (Revery 200-instead-of-404) | Blocker | Fixed | ✅ still fixed |
...
| **#15 Blocker (Revery /en/blog 500)** | Blocker (new) | Open | ✅ **FIXED** — `/en/blog` → 200, 8 articles render. Other Revery routes all 200. |
...
| `apps/revery/src/components/property/*` | Lint fails with `react-hooks/set-state-in-effect` errors in `property-filter-panel.tsx:60` and `use-location-search.ts:21`, plus 1 more | Move the `setState` out of effect body; use derived values or event handlers |
...
- Revery `Common.propertyTypes.chalet` exists only in `ro.json` (possibly intentional — `qa/revery.md`).
...
- `calculateMortgage` treats 0% interest as error and returns 0 — unrealistic market edge case (`qa/revery.md`).
...
pnpm lint:all                        # currently exits 1 — 3 errors in revery
