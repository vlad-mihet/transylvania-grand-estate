...
### [Major] Lint fails on `feature/revery` (3 errors, 3 warnings)
**Where:** `apps/revery/` — two `react-hooks/set-state-in-effect` errors plus one unshown error, 3 Next/React-hook warnings.
**Command:** `pnpm lint:all` exits 1.
**Errors:**
- `apps/revery/src/components/property/property-filter-panel.tsx:60:5` — `setSearch(searchParams.get("search") || "")` called synchronously inside `useEffect`. React 19 lint rule treats this as a cascading-render bug.
- `apps/revery/src/components/property/use-location-search.ts:21:7` — `setResults(null)` called synchronously inside `useEffect`.
- (1 more error elided by NX output truncation — likely in the same component family.)

**Warnings:**
- `apps/revery/src/components/property/map-property-popup.tsx:20:11` — `<img>` element instead of `next/image`.
- `apps/revery/src/components/property/property-map.tsx:275:6` — `useEffect` missing dependency `tMap`.
...
**Where:** `apps/revery/messages/ro.json` has the `Common.propertyTypes.chalet` key ("Cabană"); `en.json`, `fr.json`, `de.json` do not.
...
**Notes:** `docs/qa-runbook.md` calls out an intentional "Cabană" override for Revery. If intentional, the matching translations should exist for en/fr/de too (`Chalet` / `Chalet` / `Chalet` is the natural rendering in those locales). Low priority if `chalet` is a rare type in Revery inventory.
...
**Where:** `apps/revery/src/lib/calculators/mortgage.ts:24`
...
- Cross-tier slug guard works: Revery requesting a luxury slug → 404.
...
- `X-Site: REVERY` header is injected by `@tge/api-client` per `apps/revery/next.config.ts:7` (`NEXT_PUBLIC_SITE_ID: "REVERY"`). API-side tier scope binds correctly (see `qa/api.md`).
